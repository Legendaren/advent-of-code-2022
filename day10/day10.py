...
class CPU:
    ...
    def current_instruction(self):
        current_inst = self.instructions[self.instruction_counter].split()
        if len(current_inst) == 2:
            return current_inst[0], int(current_inst[1])
        return current_inst[0], None
...
def solve_part2():
    ...
    while not cpu.is_finished():
        ...
        crt_line.append('#' if pixel_pos % 40 in sprite_pos else '.')
        pixel_pos += 1
        if pixel_pos % 40 == 0:
